...
namespace LARE {
    ...
    DEVICEPREFIX void v_b_flux_body(
        const simulationData& data, 
        const remapData& remap_data,
        const volumeArray& v1_arr,
        const volumeArray& b_arr,
        const volumeArray& flux_arr,
        idx_t ix,
        idx_t iy,
        idx_t iz
    ) {
        /**
         * Writes the flux into flux
         */
    ...
    template <AxisName axis>
    DEVICEPREFIX void mass_flux_body(
        const simulationData& data,
        const remapData& remap_data,
        const volumeArray& daab_arr,
        const volumeArray& v1_arr,
        idx_t ix,
        idx_t iy,
        idx_t iz
    ...
    DEVICEPREFIX void mom_flux_core(
        const simulationData& data,
        const remapData& remap_data,
        const volumeArray& daac_arr,
        const volumeArray& v1_arr,
        const volumeArray& flux_arr,
        idx_t ix,
        idx_t iy,
        ...
        // NOTE(cmo): If we're not doing the rke_fix, then scale by dm here
        if (!data.rke) {
            flux(ia, ib, ic) *= dm(ia, ib, ic);
        }
    }

    template <AxisName axis, auto mPtr>
    DEVICEPREFIX void mom_flux_rke_fixup(
        const simulationData& data,
        const remapData& remap_data,
        const volumeArray& flux_arr,
        idx_t ix,
    ...
    template <AxisName axis, auto mPtr>
    DEVICEPREFIX void energy_flux_core(
        const simulationData& data,
        const remapData& remap_data,
        const volumeArray& daab_arr,
        const volumeArray& v1_arr,
    ...
    volumeArray slice_first_dir(const pw::acceleratedArray<fp_t, 4>& arr, int slice) {
        using pw::Range;
        auto result = arr(slice, arr.getRange(1), arr.getRange(2), arr.getRange(3));
        // NOTE(cmo): The slicing doens't seem to work properly (the lb/ub
        // should be superfluous, but I'm still getting zero lower bounds out)
        result.rebase(arr.getRange(1), arr.getRange(2), arr.getRange(3));
        return result;
    }

    template <AxisName v_comp, AxisName b_comp>
    void v_b_flux(simulationData& data, remapData& remap_data, int slice_idx) {
        PROF_PUSH("v_b_flux");
        ...
        auto flux_arr = remap_data.flux;
        if (slice_idx != -1) {
            flux_arr = slice_first_dir(remap_data.big_flux, slice_idx);
        }

        pw::applyKernel(
            LAMBDA(idx_t ix, idx_t iy, idx_t iz) {
                v_b_flux_body<v_comp, b_comp>(
                    data,
                    remap_data,
                    v1_arr,
                    b_arr,
    ...
    template <AxisName axis, auto mPtr>
    void mom_flux(simulationData& data, remapData& remap_data, int slice_idx) {
        PROF_PUSH("mom_flux");
        using Range = pw::Range;
        constexpr AxisName axis_b = AxisName((int(axis) + 1) % 3);
        constexpr AxisName axis_c = AxisName((int(axis) + 2) % 3);
        ...
        if (data.rke) {
            ...
            pw::fence();
            // NOTE(cmo): This kernel has moved inside the rke branch, as we
            // apply the scaling directly if not doing rke
            pw::applyKernel(
                LAMBDA (idx_t ix, idx_t iy, idx_t iz) {
                    flux_arr(ix, iy, iz) *= data.dm(ix, iy, iz);
                },
                Range(xs, data.nx),
                Range(ys, data.ny),
                Range(zs, data.nz)
    ...
    template <AxisName axis, auto mPtr>
    void energy_flux(simulationData& data, remapData& remap_data, int slice_idx) {
    ...
    template <AxisName axis>
    void remap_big_flux(LARE3D& lare, simulationData& data, remapData& remap_data) {
        PROF_PUSH("remap_big_flux");
        using Range = pw::Range;
        constexpr AxisName axis_b = AxisName((int(axis) + 1) % 3);
        constexpr AxisName axis_c = AxisName((int(axis) + 2) % 3);
        using RIndexer = RemapIndexer<axis, axis_b>;
        ...
        v_b_flux<axis, axis_b>(data, remap_data, 0);
        v_b_flux<axis, axis_c>(data, remap_data, 1);
        pw::fence();

        auto vbb_flux = slice_first_dir(remap_data.big_flux, 0);
        auto vbc_flux = slice_first_dir(remap_data.big_flux, 1);
        ...
        energy_flux<axis, &simulationData::energy_electron>(data, remap_data, 0);
        energy_flux<axis, &simulationData::energy_ion>(data, remap_data, 1);
        pw::fence();
        auto ee_flux = slice_first_dir(remap_data.big_flux, 0);
        auto ei_flux = slice_first_dir(remap_data.big_flux, 1);
        ...
        pw::applyKernel(
            LAMBDA(T_indexType ix, T_indexType iy, T_indexType iz) {
                ...
                remap_data.big_flux(0, ix, iy, iz) = 0.125 * (
        ...
        auto va1_temp = slice_first_dir(remap_data.big_flux, 1);
        pw::applyKernel(
            LAMBDA (idx_t ix, idx_t iy, idx_t iz) {
                const idx_t idxs[3] = {ix, iy, iz};
                const idx_t ia = idxs[int(axis)];
        ...
        auto dm_temp = slice_first_dir(remap_data.big_flux, 2);
        pw::applyKernel(
            LAMBDA (idx_t ix, idx_t iy, idx_t iz) {
                const idx_t idxs[3] = {ix, iy, iz};
                const idx_t ia = idxs[int(axis)];
        ...
        pw::assign(
            ...
            remap_data.big_flux(0, Range(0, data.nx), Range(0, data.ny), Range(0, data.nz))
        );
        pw::assign(
            va1_arr(Range(xs, xe), Range(ys, ye), Range(zs, ze)),
            va1_temp(Range(xs, xe), Range(ys, ye), Range(zs, ze))
        );
        // And copy back to dm
        pw::assign(
            data.dm(Range(xs, data.nx), Range(ys, data.ny), Range(zs, data.nz)),
            dm_temp(Range(xs, data.nx), Range(ys, data.ny), Range(zs, data.nz))
        );
        pw::fence();
        ...
        // NOTE(cmo): Right now each of these fence if we have to do an rke fix,
        // otherwise they don't.
        mom_flux<axis, &simulationData::vx>(data, remap_data, 0);
        mom_flux<axis, &simulationData::vy>(data, remap_data, 1);
        mom_flux<axis, &simulationData::vz>(data, remap_data, 2);
        auto flux_mx = slice_first_dir(remap_data.big_flux, 0);
        auto flux_my = slice_first_dir(remap_data.big_flux, 1);
        auto flux_mz = slice_first_dir(remap_data.big_flux, 2);
        pw::applyKernel(
            LAMBDA(T_indexType ix, T_indexType iy, T_indexType iz) {
                const idx_t idxs[3] = {ix, iy, iz};
                const idx_t ia = idxs[int(axis)];
    ...
    void remap(LARE3D& lare, simulationData& data, remapData& remap_data) {
        if (remap_data.use_big_flux) {
            return remap_big_flux<axis>(lare, data, remap_data);
        }
    ...
    template void v_b_flux<AxisName::X, AxisName::Y>(simulationData&, remapData&, int);
    template void v_b_flux<AxisName::X, AxisName::Z>(simulationData&, remapData&, int);
    template void v_b_flux<AxisName::Y, AxisName::X>(simulationData&, remapData&, int);
    template void v_b_flux<AxisName::Y, AxisName::Z>(simulationData&, remapData&, int);
    template void v_b_flux<AxisName::Z, AxisName::X>(simulationData&, remapData&, int);
    template void v_b_flux<AxisName::Z, AxisName::Y>(simulationData&, remapData&, int);
    ...
    template void mom_flux<AxisName::X, &simulationData::vx>(simulationData& data, remapData& remap_data, int);
    template void mom_flux<AxisName::X, &simulationData::vy>(simulationData& data, remapData& remap_data, int);
    template void mom_flux<AxisName::X, &simulationData::vz>(simulationData& data, remapData& remap_data, int);
    template void mom_flux<AxisName::Y, &simulationData::vx>(simulationData& data, remapData& remap_data, int);
    template void mom_flux<AxisName::Y, &simulationData::vy>(simulationData& data, remapData& remap_data, int);
    template void mom_flux<AxisName::Y, &simulationData::vz>(simulationData& data, remapData& remap_data, int);
    template void mom_flux<AxisName::Z, &simulationData::vx>(simulationData& data, remapData& remap_data, int);
    template void mom_flux<AxisName::Z, &simulationData::vy>(simulationData& data, remapData& remap_data, int);
    template void mom_flux<AxisName::Z, &simulationData::vz>(simulationData& data, remapData& remap_data, int);

    template void energy_flux<AxisName::X, &simulationData::energy_electron>(simulationData& data, remapData& remap_data, int);
    template void energy_flux<AxisName::X, &simulationData::energy_ion>(simulationData& data, remapData& remap_data, int);
    template void energy_flux<AxisName::Y, &simulationData::energy_electron>(simulationData& data, remapData& remap_data, int);
    template void energy_flux<AxisName::Y, &simulationData::energy_ion>(simulationData& data, remapData& remap_data, int);
    template void energy_flux<AxisName::Z, &simulationData::energy_electron>(simulationData& data, remapData& remap_data, int);
    template void energy_flux<AxisName::Z, &simulationData::energy_ion>(simulationData& data, remapData& remap_data, int);
